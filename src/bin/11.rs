...
impl Node {
    fn apply_rules_interior(&mut self) {
        match self {
            Node::Value(0) => *self = Node::Value(1),
            Node::Value(i) => {
                let mut digits = 0;
                let mut number = *i;
                ...
                if digits & 1 == 0 {
                    ...
                    *self = Node::Split {
                        left: Box::new(Node::Value(*i / d)),
                        right: Box::new(Node::Value(*i % d)),
                    };
                } else {
                    *self = Node::Value(*i * 2024);
                }
            },
            Node::Split { left, right } => {
                left.apply_rules_interior();
                right.apply_rules_interior();
            },
        }
    }

    fn apply_rules(&self) -> Node {
        match self {
            ...
            Node::Split { left, right } => Node::Split {
                left: Box::new(left.apply_rules()),
                right: Box::new(right.apply_rules()),
            },
        }
    }
}
